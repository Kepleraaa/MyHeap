...
public:
    ...
    int contains(int i,const ValueType &value);  //设计成int是为了后续两个函数都可以用
...
void Heap<ValueType>::remove(const ValueType& value){
    int x=contains(0,value);
    if (x==-1){
        error("remove: do not contain the element");
    }
    else{
        elements[x]=elements[numElements--];
        bubbleUp(x);
        bubbleDown(x);
    }
}
...
template <typename ValueType>
void Heap<ValueType>:: peekUpdated(){
    bubbleDown(0);
}
